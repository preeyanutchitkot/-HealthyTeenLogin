...
import {
  collection, query, where, onSnapshot, getDoc, doc, getDocs, limit
} from "firebase/firestore";

// 🕐 แปลง YYYY-MM-DD เป็น Date UTC
function ymdToUTCDate(ymd) { return new Date(`${(ymd || "").slice(0, 10)}T00:00:00Z`); }

// 🎨 สีหลัก
const GREEN = "#2e7d32", YELLOW = "#f9a825", RED = "#d32f2f", DARK_RED = "#b71c1c";
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!snap.exists()) {
          const qs = await getDocs(query(collection(db, "users"), where("uid", "==", uid), limit(1)));
        ...
        todayDocs.forEach(d => {
          const x = d.data();
          totalToday += (Number(x?.calories ?? x?.cal ?? 0) || 0) * (Number(x?.qty ?? 1) || 1);
        });
        ...
        const ymdYesterday = new Intl.DateTimeFormat("en-CA", { timeZone: tz }).format(yesterday);
        ...
        yestDocs.forEach(d => {
          const x = d.data();
          totalYest += (Number(x?.calories ?? x?.cal ?? 0) || 0) * (Number(x?.qty ?? 1) || 1);
        });
        ...
        const diffPct = totalYest > 0 ? ((totalToday - totalYest) / totalYest) * 100 : 0;
        ...
        unsub = onSnapshot(qFoodToday, (snap) => {
          ...
          snap.forEach(d => {
            const x = d.data();
            total += (Number(x?.calories ?? x?.cal ?? 0) || 0) * (Number(x?.qty ?? 1) || 1);
          });
          setTodayCal(total);
        ...
        if (ymd === todayYMD) {
          ...
          const th = new Intl.DateTimeFormat("th-TH-u-ca-buddhist", { day: "2-digit", month: "2-digit", year: "numeric" });
        ...
        <div className="ring-left">
          <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
            ...
            <circle cx={size / 2} cy={size / 2} r={radius} fill="none"
              stroke={trackColor} strokeWidth={thickness} strokeLinecap="round" />
